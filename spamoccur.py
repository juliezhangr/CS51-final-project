...
import sqlite
    ...
    def load ():
        try:
            ...
            cursor.execute('CREATE TABLE spamoccur (token STRING PRIMARY KEY, occurences FLOAT)')
    ...
    def update (token, o):
        t = "'" + token + "'"
        statement = token + ',' + 
        cursor.execute('REPLACE INTO spamoccur (token, occurences) VALUES (' + statement + ')')
    
    # removes word entry from database (string -> unit)
    def remove (t):
        t = "'" + token + "'"
        cursor.execute('DELETE * FROM spamoccur WHERE token = ' + t )

    # gets the spam probability given a token (string -> float)
    def getOccur (token):
        t = "'" + token + "'"
        cursor.execute('SELECT occurences FROM spamoccur WHERE token =' + t)
